...
approx_normal_filename <- "approx_normal_depdenency.png"
...
nr_not_na = dcors_df |> group_by(name) |> summarize(nr_na=is.na(dcorT) %>% sum(), nr_not_na = (!is.na(dcorT_Q)) %>% sum())
...
create_dependency_rejection_table <- function(df) {
  a = df |> filter(!is.na(p_value_Q)) |> filter(D_tilde==1) |> mutate(dcor_T_p_value = p_value_Q)
...
table_rejection = create_dependency_rejection_table(dcors_df_rejection)

...
create_copula_summary_table <- function(df) {
  ...
  df  |> group_by(setting_pretty) |> summarise(
    ...
    percentage_over_1=num_higher/n(),
    percentage_under_1=num_lower/n(),
...
filtered |>  create_copula_summary_table() |> 
  arrange(percentage_under_1 - percentage_over_1) |> head(5)
...
over_lower_one_table <- filtered  |>  create_copula_summary_table() |> 
  mutate(across(c(percentage_over_1:percentage_equal, q_0.25, median, q_0.75), ~ signif(.x, 3))) |> 
  arrange(percentage_under_1 - percentage_over_1) |>
  select(setting_pretty, percentage_over_1, percentage_under_1, q_0.25, median, q_0.75)
# save over_lower_one_table to csv
write_csv(over_lower_one_table, "output/copula_estimation_over_under_one_table.csv")
over_lower_one_table 

...
```{r}
study <- filter_study_keep_ids(studies, 4)[[1]]
test_question_id <- 10
...
```
```{r}
error_margins[1:7] |> plot_distributions("pdf")
```
```{r}
error_margins[[7]]$pdf(0.1)
```
...
```
```{r}
integrate(function(x) exp(log_posterior(x)), support[1], support[2], subdivisions = 10000)
```
```{r}

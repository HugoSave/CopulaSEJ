...
realizations <- training_set |> study_df_to_realizations()
cdf_metric <- get_CDF_decoupler(scale="linear", support_restriction = NULL)
rel_metric <- get_relative_decoupler(1, compose_sigmoid=TRUE, m_preprocess="median", k=0.05)
...
```{r}
JC_model <- fit_JC_model(training_assessments, test_m_matrix, realizations)
y_JC <- JC_model$posterior$logDM(x)
plot(x, exp(y_JC), type="l", col="red", lwd=2, main="JC model vs posterior")
```
```{r}
y_DP <- posterior_product_predictor(test_m_matrix)$posterior$logDM(x)
plot(x, exp(y_DP), type="l", col="blue", lwd=2, main="DP model vs posterior")
```

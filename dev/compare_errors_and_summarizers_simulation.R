...
run_study <- function(studies, error_metric, summarizing_function, prediction_methd="copula"){
...
if (!interactive()) {
  ...
  data_list_short <- list_mod[18]
  results <- list()
  warnings <- list()

  res <- run_study(data_list_short, get_CDF_indep_class(), get_three_quantiles_summarizing_function())

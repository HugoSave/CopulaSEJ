...
nr_filtered_points <- results_df |> filter(realization!=0 & abs(rel_error) < max_remax_rel_error_in_plt) |> nrow()
nr_away_filtered_points <- results_df |> nrow() - nr_filtered_points
# lowest number of points for a study
min_points <- study_data |> filter(realization!=0 & abs(rel_error) < max_remax_rel_error_in_plt) |> group_by(study_id, prediction_method) |>
